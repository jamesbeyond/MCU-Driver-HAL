...
#error \
    "Unsupported error filename buffer length detected, max supported length is 64 chars. Please change MBED_CONF_PLATFORM_MAX_ERROR_FILENAME_LEN or max-error-filename-len in configuration."
#endif
#endif
...
#define MAKE_MBED_ERROR(type, module, error_code)                                                                   \
    (mbed_error_status_t)(                                                                                          \
        (0x80000000) |                                                                                              \
...
#define MBED_GET_ERROR_MODULE(error_status) \
    ((error_status & MBED_ERROR_STATUS_MODULE_MASK) >> MBED_ERROR_STATUS_MODULE_POS)
#define MBED_GET_ERROR_CODE(error_status)                              \
    (int)((MBED_GET_ERROR_TYPE(error_status) == MBED_ERROR_TYPE_POSIX) \
              ? (-error_status)                                        \
              : ((error_status & MBED_ERROR_STATUS_CODE_MASK) >> MBED_ERROR_STATUS_CODE_POS))

/** mbed_error_status_t description
 *
 * mbed_error_status_t type represents the error status values under MbedOS. mbed_error_status_t values are signed
 integers and always be negative.\n
 ...
 *   POSIX Error Status-es  - 0xFFFFFFFF to 0xFFFFFF01(-1 -255) - This corresponds to POSIX error codes represented as
 negative.\n
 *   System Error Status-es - 0x80XX0100 to 0x80XX0FFF - This corresponds to System error codes range(all values are
 negative). Bits 23-16 will be module type(marked with XX)\n
 *   Custom Error Status-es - 0xA0XX1000 to 0xA0XXFFFF - This corresponds to Custom error codes range(all values are
 negative). Bits 23-16 will be module type(marked with XX)\n\n
 *
 * The ERROR CODE(values encoded into ERROR CODE bit-field in mbed_error_status_t) value range for each error type is
 also separated as below:\n
 *   POSIX Error Codes  - 1 to 255.\n
 *   System Error Codes - 256 to 4095.\n
 *   Custom Error Codes - 4096 to 65535.\n
 *
 * @note POSIX error codes are always encoded as negative of their actual value. For example, EPERM is encoded as
 -EPERM.
 *       And, the MODULE TYPE for POSIX error codes are always encoded as MBED_MODULE_UNKNOWN.\n
 *       This is to enable easy injection of POSIX error codes into MbedOS error handling system without altering the
 actual POSIX error values.\n
...
/**
 * Macro for defining a POSIX error status. This macro is mainly used to define POSIX error values in mbed_error_code_t
 * enumeration.
...
#define MBED_DEFINE_POSIX_ERROR(error_name, error_code) \
    MBED_ERROR_CODE_##error_name = error_code, MBED_ERROR_##error_name = -(MBED_POSIX_ERROR_BASE + error_code)

/**
 * Macro for defining a System error status. This macro is used to define System error values in mbed_error_code_t
 * enumeration.
 * @param  error_name       Name of the error without the ERROR_ prefix
...
#define MBED_DEFINE_SYSTEM_ERROR(error_name, error_code)                \
    ...
    MBED_ERROR_##error_name =                                           \
        MAKE_MBED_ERROR(MBED_ERROR_TYPE_SYSTEM, MBED_MODULE_UNKNOWN, MBED_ERROR_CODE_##error_name)

/**
 * Macro for defining a Custom error status. This macro is used to define custom error values in mbed_error_code_t
 * enumeration.
 * @param  error_name       Name of the error without the ERROR_ prefix
...
#define MBED_DEFINE_CUSTOM_ERROR(error_name, error_code)                \
    ...
    MBED_ERROR_##error_name =                                           \
        MAKE_MBED_ERROR(MBED_ERROR_TYPE_CUSTOM, MBED_MODULE_UNKNOWN, MBED_ERROR_CODE_##error_name)

/**
 * Macros for setting a system warning. These macros will log the error, Its a wrapper for calling mbed_warning API.
 * There are 2 versions of this macro. MBED_WARNING takes status and message. MBED_WARNING1 takes an additional context
 * specific argument
 * @param  error_status     mbed_error_status_t status to be set(See mbed_error_status_t enum above for available error
 * status values).
 ...
 *        Since this macro is a wrapper for mbed_warning API callers should process the return value from this macro
 * which is the return value from calling mbed_error API.
 *
 */
#ifdef NDEBUG
#define MBED_WARNING1(error_status, error_msg, error_value) \
    mbed_warning(error_status, (const char *)NULL, (uint32_t)error_value, NULL, 0)
#define MBED_WARNING(error_status, error_msg) mbed_warning(error_status, (const char *)NULL, (uint32_t)0, NULL, 0)
#else // NDEBUG
#if MBED_CONF_PLATFORM_ERROR_FILENAME_CAPTURE_ENABLED
#define MBED_WARNING1(error_status, error_msg, error_value) \
    mbed_warning(error_status, (const char *)error_msg, (uint32_t)error_value, (const char *)MBED_FILENAME, __LINE__)
#define MBED_WARNING(error_status, error_msg) \
    mbed_warning(error_status, (const char *)error_msg, (uint32_t)0, (const char *)MBED_FILENAME, __LINE__)
...
/**
 * Macros for setting a fatal system error. These macros will log the error, prints the error report and halts the
 * system. Its a wrapper for calling mbed_error API. There are 2 versions of this macro. MBED_ERROR takes status and
 * message. MBED_ERROR1 takes an additional context specific argument
 * @param  error_status     mbed_error_status_t status to be set(See mbed_error_status_t enum above for available error
 * status values).
 * @param  error_msg        The error message to be printed out to STDIO/Serial.
 * @param  error_value      Value associated with the error status. This would depend on error code/error scenario. Only
 * available with MBED_ERROR1
 ...
 *       Since this macro is a wrapper for mbed_error API callers should process the return value from this macro which
 * is the return value from calling mbed_error API.
 *
 */
#ifdef NDEBUG
#define MBED_ERROR1(error_status, error_msg, error_value) \
    mbed_error(error_status, (const char *)NULL, (uint32_t)error_value, NULL, 0)
#define MBED_ERROR(error_status, error_msg) mbed_error(error_status, (const char *)NULL, (uint32_t)0, NULL, 0)
#else // NDEBUG
#if MBED_CONF_PLATFORM_ERROR_FILENAME_CAPTURE_ENABLED
#define MBED_ERROR1(error_status, error_msg, error_value) \
    mbed_error(error_status, (const char *)error_msg, (uint32_t)error_value, (const char *)MBED_FILENAME, __LINE__)
#define MBED_ERROR(error_status, error_msg) \
    mbed_error(error_status, (const char *)error_msg, (uint32_t)0, (const char *)MBED_FILENAME, __LINE__)
#else // MBED_CONF_PLATFORM_ERROR_FILENAME_CAPTURE_ENABLED
#define MBED_ERROR1(error_status, error_msg, error_value) \
 ...
 *  @note
 *  This enumeration defines the Error types supported. The value of these enum values will be encoded into
 * mbed_error_status_t TYPE field.\n See mbed_error_status_t description for more info.\n MBED_ERROR_TYPE_SYSTEM - Used
 * to indicate that the error status is of System defined Error type.\n MBED_ERROR_TYPE_CUSTOM - Used to indicate that
 * the error status is of Custom defined Error type.\n MBED_ERROR_TYPE_POSIX  - Used to indicate that the error status
 * is of POSIX error type.\n
 *
 */
typedef enum _mbed_error_type_t {
    ...
    // and thus we must use 3 to match the type bits in error status representation which are from 0xFFFFFFFF to
    // 0xFFFFFF00
 ...
 *      ERROR_CODE_EPERM = 1\n
 *      ERROR_EPERM = -1\n
 *  All POSIX error codes currently supported by MbedOS(defined in mbed_retarget.h) are defined using the
 MBED_DEFINE_POSIX_ERROR macro.\n\n
 ...
 *  New System Error codes should be defined using MBED_DEFINE_SYSTEM_ERROR macro and must have an unique error code
 value\n
 ...
 *  @note
 *  **Using error codes:** \n
 *  POSIX error codes may be used in modules/functions currently using POSIX error codes and switching them to Mbed-OS
 error codes
 *  may cause interoperability issues. For example, some of the filesystem, network stack implementations may need to
 use
 *  POSIX error codes in order to keep them compatible with other modules interfacing with them, and may continue to use
 POSIX error codes.
 *
 ...
 *  If you get an error report as below which you want to search for in mbed-os source tree, first take note of "Error
 Code" number. \n
 *  For example, the below error report has an error code of \b 259. Find the error name associated with the error code
 and in this case its \b INVALID_FORMAT. \n
 *  Use that error name(\b INVALID_FORMAT) to search the source tree for code locations setting that specific error
 code. \n
 *  If the Error module reported is not 255(which indicates unknown module), you can also use that to narrow down to the
 specific component reporting the error.
...
typedef enum _mbed_error_code {
    ...
    MBED_DEFINE_SYSTEM_ERROR(BLE_USE_INCOMPATIBLE_API,
                             71),                    /* 327      Concurrent use of incompatible versions of a BLE API */
...
typedef struct _mbed_error_ctx {
    ...
    int32_t error_reboot_count;  // everytime we write this struct we increment this value by 1, irrespective of time
                                 // between reboots. Note that the data itself might change, but everytime we reboot due
                                 // to error we update this count by 1
 ...
 * @param  module           Module generating the error code. If its unknown, pass MBED_MODULE_UNKNOWN. See
 * mbed_module_type_t for module types.
 * @param  error_code       The mbed_error_code_t code to be used in generating the mbed_error_status_t. See
 * mbed_error_code_t for error codes.
 *
 * @code
 *
...
/**
 * Callback/Error hook function prototype. Applications needing a callback when an error is reported can use
 * mbed_set_error_hook function to register a callback/error hook function using the following prototype. When an error
 * happens in the system error handling implementation will invoke this callback with the mbed_error_status_t reported
 * and the error context at the time of error.
 ...
 * mbed_error_hook function should be overridden with custom implementation. When an error happens in the system error
 * handling implementation will invoke this callback with the mbed_error_status_t reported and the error context at the
 * time of error.
 * @param  error_ctx        Error context structure associated with this error.
 * @return                  void
 *
 ...
 * error info in special crash data RAM region. Once rebooted, MbedOS initialization routines will call this function
 * with a pointer to the captured mbed_error_ctx structure. If application implementation needs to receive this
 * callback, mbed_error_reboot_callback function should be overridden with custom implementation. By default it's
 * defined as a WEAK function in mbed_error.c. Note that this callback will be invoked before the system starts
 * executing main() function. So the implementation of the callback should be aware any resource
 * limitations/availability of resources which are yet to be initialized by application main().
 *
 * @param  error_ctx        Error context structure associated with this error.
 * @return                  void
 *
...
/**
 * Initialize error handling system, this is called by the mbed-os boot sequence. This is not required to be called by
 * Application unless the boot sequence is overridden by the system implementation. NOTE: If
 * MBED_CONF_PLATFORM_FATAL_ERROR_AUTO_REBOOT_ENABLED is enabled and if the current reboot count exceeds
 * MBED_CONF_PLATFORM_ERROR_REBOOT_MAX the system will halt when this function is called, and in such cases the caller
 * will not get the control back. Also note that calling this function may trigger mbed_error_reboot_callback() if
 * application side overides mbed_error_reboot_callback().
...
/**
 * Call this function to retrieve the error context after a fatal error which triggered a system reboot. The function
 * retrieves the error context stored in crash-report ram area which is preserved over reboot.
 * @param  error_info           Pointer to mbed_error_ctx struct allocated by the caller. This is the mbed_error_ctx
 * info captured as part of the fatal error which triggered the reboot.
...
/**
 * Calling this function resets the current reboot context captured by the system(stored in special crash data RAM
 * region).
 * @return                  MBED_SUCCESS on success.
 *                          MBED_ERROR_ITEM_NOT_FOUND if no reboot context is currently captured by the system
 */
...
/**
 * Calling this function resets the current reboot count stored as part of error context captured in special crash data
 * RAM region. The function will also update the CRC value stored as part of error context accordingly.
 * @return                  MBED_SUCCESS on success.
 *                          MBED_ERROR_ITEM_NOT_FOUND if no reboot context is currently captured by the system
 */
...
/**
 * Call this function to set a system error/warning. This function will log the error status with the context info and
 * return to caller.
 *
 * @param  error_status     mbed_error_status_t status to be set(See mbed_error_status_t enum above for available error
 * status values).
 ...
 * @param  line_number      The line number of the source file originating the error( Most callers can pass __LINE__
 * here ) .
...
/**
 * Call this function to set a fatal system error and halt the system. This function will log the fatal error with the
 * context info and prints the error report and halts the system.
 *
 * @param  error_status     mbed_error_status_t status to be set(See mbed_error_status_t enum above for available error
 * status values).
 * @param  error_msg        The error message to be printed out to STDIO/Serial.
 ...
 * NOTE: This function should be implemented for re-entrancy as multiple threads may invoke mbed_error which may cause
 * error hook to be called.
 * @param  custom_error_hook    mbed_error_status_t status to be set(See mbed_error_status_t enum above for available
 * error status values).
 * @return                      0 or MBED_SUCCESS on success.
 ...
 * @param  error_info           This is the mbed_error_context info captured as part of the first mbed_error call. The
 * caller should pass a pointer to mbed_error_context struct allocated by the caller.
 * @return                      0 or MBED_SUCCESS on success.
 ...
 * @param  error_info           This is the mbed_error_context info captured as part of the last mbed_error call. The
 * caller should pass a pointer to mbed_error_context struct allocated by the caller.
 ...
 */
mbed_error_status_t
mbed_make_error(mbed_error_type_t error_type, mbed_module_type_t module, mbed_error_code_t error_code);

/**
 * Returns the current number of entries in the error history, if there has been more than max number of errors logged
 * the number returned will be max depth of error history.
 ...
 *                              The number of entries in the error history is configured during build and the max index
 * depends on max depth of error history.\n index = 0 points to the oldest entry in the history, and index = (max
 * history depth - 1) points to the latest entry in the error history.\n
 * @param  error_info           This is the mbed_error_context info captured as part of the error history. The caller
 * should pass a pointer to mbed_error_context struct allocated by the caller.
 * @return                      0 or MBED_SUCCESS on success.
 *                              MBED_ERROR_INVALID_ARGUMENT in case of invalid index
 *
 */
