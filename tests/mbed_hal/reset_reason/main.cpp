...
typedef enum { CMD_STATUS_CONTINUE, CMD_STATUS_ERROR } cmd_status_t;
...
static cmd_status_t handle_command(const char *key, const char *value)
{
    if (strcmp(key, MSG_KEY_RESET_REASON_RAW) == 0) {
        ...
        int      raw_reason_hex_str_len = snprintf(raw_reason_hex_str, sizeof raw_reason_hex_str, "%08lx", raw_reason);
