...
#include "cmsis.h"
    ...
    defined(__MPU_PRESENT) && (__MPU_PRESENT == 1U) && !defined(MBED_MPU_CUSTOM)
...
static_assert(MBED_MPU_ROM_END <= 0x20000000 - 1, "Unsupported value for MBED_MPU_ROM_END");
...
void mbed_mpu_init()
{
     ...
     * 0x00000000 - 0x1FFFFFFF     Code            Yes                      WT, WA              Write disabled for first
     * portion and execute disabled for the rest 0x20000000 - 0x3FFFFFFF     SRAM            Yes WB, WA, RA Execute
     * disabled 0x40000000 - 0x5FFFFFFF     Peripheral      No 0x60000000 - 0x7FFFFFFF     RAM             Yes WB, WA,
     * RA          Execute disabled 0x80000000 - 0x9FFFFFFF     RAM             Yes                      WT, RA Execute
     * disabled 0xA0000000 - 0xBFFFFFFF     Device          No 0xC0000000 - 0xDFFFFFFF     Device          No 0xE0000000
     * - 0xFFFFFFFF     System          No
     */
    ...
    ARM_MPU_SetRegion(0,                             // Region
                      ARM_MPU_RBAR(0x00000000,       // Base
                                   ARM_MPU_SH_NON,   // Non-shareable
                                   1,                // Read-Only
                                   1,                // Non-Privileged
                                   0),               // Execute Never disabled
                      ARM_MPU_RLAR(MBED_MPU_ROM_END, // Limit
                                   AttrIndex_WTRA)   // Attribute index - Write-Through, Read-allocate
    );

#if MBED_MPU_RAM_START != 0x20000000
    ARM_MPU_SetRegion(4,                               // Region
                      ARM_MPU_RBAR(MBED_MPU_RAM_START, // Base
                                   ARM_MPU_SH_NON,     // Non-shareable
                                   0,                  // Read-Write
                                   1,                  // Non-Privileged
                                   1),                 // Execute Never enabled
                      ARM_MPU_RLAR(0x1FFFFFFF,         // Limit
                                   AttrIndex_WTRA)     // Attribute index - Write-Through, Read-allocate
    );
...
#define LAST_RAM_REGION 3
    ...
    ARM_MPU_SetRegion(3,                           // Region
                      ARM_MPU_RBAR(0x80000000,     // Base
                                   ARM_MPU_SH_NON, // Non-shareable
                                   0,              // Read-Write
                                   1,              // Non-Privileged
                                   1),             // Execute Never enabled
                      ARM_MPU_RLAR(0x9FFFFFFF,     // Limit
                                   AttrIndex_WTRA) // Attribute index - Write-Through, Read-allocate
    );
