...
#if ((__ARM_ARCH_7M__ == 1U) || (__ARM_ARCH_7EM__ == 1U) || (__ARM_ARCH_6M__ == 1U)) && defined(__MPU_PRESENT) && \
    (__MPU_PRESENT == 1U) && !defined(MBED_MPU_CUSTOM)
...
static_assert(MBED_MPU_ROM_END == 0x04000000 - 1 || MBED_MPU_ROM_END == 0x08000000 - 1 ||
                  MBED_MPU_ROM_END == 0x0C000000 - 1 || MBED_MPU_ROM_END == 0x10000000 - 1 ||
                  MBED_MPU_ROM_END == 0x14000000 - 1 || MBED_MPU_ROM_END == 0x18000000 - 1 ||
                  MBED_MPU_ROM_END == 0x1C000000 - 1 || MBED_MPU_ROM_END == 0x20000000 - 1,
...
void mbed_mpu_init()
{
     ...
     * 0x00000000 - 0x1FFFFFFF     Code            Yes                          Write disabled for first portion and
     * execute disabled for the rest 0x20000000 - 0x3FFFFFFF     SRAM            Yes                          Execute
     * disabled 0x40000000 - 0x5FFFFFFF     Peripheral      No 0x60000000 - 0x7FFFFFFF     RAM             Yes Execute
     * disabled 0x80000000 - 0x9FFFFFFF     RAM             Yes                          Execute disabled 0xA0000000 -
     * 0xBFFFFFFF     Device          No 0xC0000000 - 0xDFFFFFFF     Device          No 0xE0000000 - 0xFFFFFFFF System
     * No
     */
    ...
    ARM_MPU_SetRegion(
        ...
        ARM_MPU_RASR(
            ...
            ((MBED_MPU_ROM_END >= 0x00000000) ? 0 : (1 << 0)) | // 0 to enable, 1 << n to disable
                ((MBED_MPU_ROM_END >= 0x04000000) ? 0 : (1 << 1)) | ((MBED_MPU_ROM_END >= 0x08000000) ? 0 : (1 << 2)) |
                ((MBED_MPU_ROM_END >= 0x0C000000) ? 0 : (1 << 3)) | ((MBED_MPU_ROM_END >= 0x10000000) ? 0 : (1 << 4)) |
                ((MBED_MPU_ROM_END >= 0x14000000) ? 0 : (1 << 5)) | ((MBED_MPU_ROM_END >= 0x18000000) ? 0 : (1 << 6)) |
    ...
    ARM_MPU_SetRegion(
        ARM_MPU_RBAR(3,               // Region
                     0x00000000),     // Base
        ARM_MPU_RASR(1,               // DisableExec
...
#define LAST_RAM_REGION 2
    ...
    ARM_MPU_SetRegion(ARM_MPU_RBAR(1,               // Region
                                   0x00000000),     // Base
                      ARM_MPU_RASR(1,               // DisableExec
                                   ARM_MPU_AP_FULL, // AccessPermission
                                   1,               // TypeExtField
    ...
    ARM_MPU_SetRegion(ARM_MPU_RBAR(2,                         // Region
                                   0x80000000),               // Base
                      ARM_MPU_RASR(1,                         // DisableExec
                                   ARM_MPU_AP_FULL,           // AccessPermission
                                   0,                         // TypeExtField
    ...
    // Enable the MPU
    MPU->CTRL = (1 << MPU_CTRL_PRIVDEFENA_Pos) | // Use the default memory map for unmapped addresses
