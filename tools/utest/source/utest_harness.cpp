...
DefaultHandlers  defaults_;
DefaultHandlers  handlers_;
DefaultHandlers *defaults = &defaults_;
DefaultHandlers *handlers = &handlers_;
...
void Harness::raise_failure(const failure_reason_t reason)
{
    ...
    if (fail_status == STATUS_ABORT || reason & REASON_CASE_SETUP) {
        if (handlers->case_teardown && location != LOCATION_CASE_TEARDOWN) {
            ...
            utest::v1::status_t teardown_status =
                handlers->case_teardown(case_current, case_passed, case_failed, failure_t(reason, fail_loc));
...
void Harness::schedule_next_case()
{
    ...
    if (case_control.repeat & REPEAT_SETUP_TEARDOWN ||
        !(case_control.repeat & (REPEAT_ON_TIMEOUT | REPEAT_ON_VALIDATE))) {
        location = LOCATION_CASE_TEARDOWN;

        if (handlers->case_teardown) {
            utest::v1::status_t status = handlers->case_teardown(case_current,
                                                                 case_passed,
                                                                 case_failed,
                                                                 case_failed ? failure_t(REASON_CASES, LOCATION_UNKNOWN)
                                                                             : failure_t(REASON_NONE));
...
void Harness::handle_timeout()
{
    ...
    if (case_timeout_occurred) {
        raise_failure(
            failure_reason_t(REASON_TIMEOUT | ((case_control.repeat & REPEAT_ON_TIMEOUT) ? REASON_IGNORE : 0)));
...
void Harness::run_next_case()
{
    ...
    if (case_current < (test_cases + test_length)) {
        ...
        handlers->test_teardown(
            test_passed, test_failed, test_failed ? failure_t(REASON_CASES, LOCATION_UNKNOWN) : failure_t(REASON_NONE));
        test_cases = NULL;
        exit(test_failed);
    } else {
        exit(test_failed);
