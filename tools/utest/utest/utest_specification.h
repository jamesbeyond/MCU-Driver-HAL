...
class Specification
{
  public:
    ...
    Specification(const CaseType (&cases)[N], const handlers_t defaults = default_handlers)
        : setup_handler(default_handler), teardown_handler(default_handler), failure_handler(default_handler),
          cases(static_cast<const Case *>(static_cast<const CaseType *>(cases))), length(N), defaults(defaults)
    {
        static_assert(sizeof(CaseType) == sizeof(Case), "CaseType and Case should have the same size");
    }
    ...
    Specification(const test_setup_handler_t    setup_handler,
                  ...
                  const handlers_t              defaults = default_handlers)
        : setup_handler(setup_handler), teardown_handler(teardown_handler), failure_handler(failure_handler),
          cases(cases), length(length), defaults(defaults)
    {
    }
