...
enum repeat_t {
    ...
    REPEAT_ALL_ON_TIMEOUT =
        REPEAT_SETUP_TEARDOWN | REPEAT_ON_TIMEOUT, ///< repeat the handler with setup and teardown on timeout
...
struct failure_t {
    ...
    failure_t ignored() const { return failure_t(failure_reason_t(reason | REASON_IGNORE), location); }
...
struct base_control_t {
    ...
    repeat_t inline get_repeat() const { return repeat; }
    uint32_t inline get_timeout() const { return timeout; }
};
...
struct control_t : private base_control_t {
    ...
    control_t(repeat_t repeat, uint32_t timeout_ms) : base_control_t(make_base_control_t(repeat, timeout_ms)) {}

    control_t(repeat_t repeat) : base_control_t(make_base_control_t(repeat, TIMEOUT_UNDECLR)) {}

    control_t(uint32_t timeout_ms) : base_control_t(make_base_control_t(REPEAT_UNDECLR, timeout_ms)) {}

    control_t(const base_control_t &other) : base_control_t(other) {}

    friend control_t operator+(const control_t &lhs, const control_t &rhs)
    {
        control_t result(repeat_t(lhs.repeat | rhs.repeat), (rhs.timeout == TIMEOUT_NONE) ? rhs.timeout : lhs.timeout);
  ...
  private:
    ...
    {
        base_control_t result = {repeat, timeout};
        return result;
    }
...
/// @see operator+ in control_t
inline control_t operator+(const base_control_t &lhs, const control_t &rhs) { return control_t(lhs) + rhs; }

/// @see operator+ in control_t
inline control_t operator+(const control_t &lhs, const base_control_t &rhs) { return lhs + control_t(rhs); }
 ...
 */
typedef utest::v1::status_t (*case_teardown_handler_t)(const Case *const source,
                                                       const size_t      passed,
                                                       const size_t      failed,
...
// deprecations
__deprecated_message("Use CaseRepeatAll instead.") extern const base_control_t CaseRepeat;

__deprecated_message("Use CaseRepeatHandler instead.") extern const base_control_t CaseRepeatHandlerOnly;
...
__deprecated_message("Use REASON_CASE_TEARDOWN instead.") const failure_reason_t FAILURE_TEARDOWN =
